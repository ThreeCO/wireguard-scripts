...
sudo iptables -A $CHAIN_NAME -s $WIREGUARD_LAN -i $WIREGUARD_INTERFACE -p tcp -j FW_WG_TCP
sudo iptables -A $CHAIN_NAME -s $WIREGUARD_LAN -i $WIREGUARD_INTERFACE -p udp -j FW_WG_UDP

# Accept traffic from any Wireguard IP address connected to the Wireguard server
sudo iptables -A $CHAIN_NAME -s $WIREGUARD_LAN -i $WIREGUARD_INTERFACE -j ACCEPT
